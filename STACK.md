...
| 1 | test_volprim_rf_basic::test03_render_depth[scalar_rgb-ellipsoids] | **upstream bug**: `ShapeKDTree::intersect_prim` (kdtree.h) calls `shape->ray_intersect_preliminary_scalar(ray)` / `ray_test_scalar(ray)` without the local prim index for non-mesh shapes, and the default is `prim_index = 0` -- so a multi-primitive analytic `ellipsoids` shape only ever intersects primitive 0 | probe: 3 identity-rotation ellipsoids, per-primitive kd-tree bboxes verified correct, yet only prim 0 is hittable; scalar scene finds 32/3205 of the hits OptiX finds on the same ray grid; depth image is ~0 where the ref has ~4.8; kdtree.h byte-identical to upstream master (`git diff 3989175f..HEAD` empty), call site last touched by upstream a291652b | pre-existing upstream bug, invisible in upstream CI (Embree default ON); exposed by Embree-off |

The failing volprim run wrote diagnostic EXRs
(`volprim_rf_basic_depth[ellipsoids]_{pvalue,success,image}.exr`) into the
`resources/data` submodule's refs directory; they are untracked debris of the
failed statistical test, not references.
