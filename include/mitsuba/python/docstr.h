...
R"doc(Longest visualized path depth (``-1`` = infinite).
A value of ``1`` will visualize only directly visible light sources.
``2`` will lead to single-bounce (direct-only) illumination, and so on.)doc";
...
If set to (size_t) -1, all the work is done in a single pass (default).)doc";

static const char *__doc_mitsuba_AdjointIntegrator_render = R"doc()doc";

static const char *__doc_mitsuba_AdjointIntegrator_sample =
R"doc(Sample the incident importance and splat the product of
...
not thread-safe (i.e., it may not be copied concurrently).)doc";
...
R"doc(This class defines an abstract destination
for logging-relevant information)doc";
...
Args:
    ...
    ptr: Custom pointer payload. This is used to express the
        context of a progress message.)doc";
...
.. code-block:: python
   ...
   if arg1:
       print("Got -f", arg1.as_string()))doc";
...
Args:
    ...
    extra: Indicates whether the argument accepts an extra argument value)doc";
...
R"doc(For arguments that are specified multiple times, advance to
the next one.)doc";
...
static const char *__doc_mitsuba_BSDFContext_component = R"doc(Integer value of requested BSDF component index to be sampled/evaluated.)doc";

static const char *__doc_mitsuba_BSDFContext_is_enabled =
R"doc(Checks whether a given BSDF component type and index are
...
This updates the transport mode (radiance to importance and vice versa).)doc";

static const char *__doc_mitsuba_BSDFContext_type_mask =
...
R"doc(Any reason the shading frame must carry a smooth tangent rather
than an arbitrary basis around the shading normal)doc";
...
R"doc(The lobe lives in a shading frame that a normal map perturbs, so
the frame's orientation around the normal is observable)doc";

static const char *__doc_mitsuba_BSDFFlags_Null = R"doc('null' scattering event, i.e. particles do not undergo deflection)doc";

static const char *__doc_mitsuba_BSDFFlags_Reflection = R"doc(Any reflection component (scattering into discrete, 1D, or 2D set of directions))doc";
...
static const char *__doc_mitsuba_BSDFFlags_Transmission = R"doc(Any transmission component (scattering into discrete, 1D, or 2D set of directions))doc";
...
R"doc(Given a surface interaction and an incident/exitant direction
...
R"doc(Evaluate the BSDF f(wi, wo) or its adjoint version :math:`f^{*}(wi, wo)`
...
R"doc(Monochromatic evaluation of a BSDF attribute at the given surface interaction
...
R"doc(Trichromatic evaluation of a BSDF attribute at the given surface interaction
...
R"doc(Jointly evaluate the BSDF f(wi, wo), the probability per unit
...
R"doc(Compute the probability per unit solid angle of sampling a
given direction
...
static const char *__doc_mitsuba_BaseSunskyEmitter_bbox = R"doc(This emitter does not occupy any particular region of space, return an invalid bounding box)doc";
...
static const char *__doc_mitsuba_BaseSunskyEmitter_eval =
R"doc(This method should be used when the decision of hitting the sun was already made.
...
static const char *__doc_mitsuba_BaseSunskyEmitter_eval_2 = R"doc()doc";
...
Returns:
    The buffer with the extracted albedo values for the needed wavelengths/channels)doc";
...
Returns:
    The parameters associated with the mixed pdf sampling of the given elevation)doc";

static const char *__doc_mitsuba_BaseSunskyEmitter_get_sky_datasets =
R"doc(Getter for sky radiance datasets for the given wavelengths and sun angles
...
Returns:
    ...
    for the Wilkie-Hosek 2012 sky model)doc";

static const char *__doc_mitsuba_BaseSunskyEmitter_get_sky_sampling_weight =
R"doc(Getter for the probability of sampling the sky for a given sun position
...
R"doc(Getter for the sun's irradiance at a given sun elevation and wavelengths
...
R"doc(Compute the elevation and azimuth of the sun as seen by an observer
...
Returns:
    The cosine of the angle between the sun's radius and the viewing direction)doc";
...
R"doc(General-purpose bitmap class with read and write support
...
the class methods and enumerations for further detail.)doc";
...
Args:
    ...
    channel_names: Channel names of the image. This parameter is optional, and only used
        when ``pixel_format`` = `PixelFormat.MultiChannel`)doc";
...
Args:
    ...
    format: File format to be read (PNG/EXR/Auto-detect ...))doc";
...
.. note::
   ...
   the source stream must support the `Stream.seek()` operation.)doc";
...
* Loading and saving of 8 bit per component RGB and
  luminance bitmaps)doc";

static const char *__doc_mitsuba_Bitmap_FileFormat_OpenEXR =
R"doc(OpenEXR high dynamic range file format developed by
...
R"doc(This enumeration lists all pixel format types supported
by the `Bitmap` class. This both determines the
number of channels, and how they should be interpreted)doc";
...
R"doc(Accumulate the contents of another bitmap into the
...
Note:
    ...
    use different component formats or channels.)doc";
...
static const char *__doc_mitsuba_Bitmap_metadata_2 = R"doc(Return a `Properties` object containing the image metadata (const version))doc";
...
Args:
    ...
    temp: Optional: image for intermediate computations)doc";
...
Args:
    ...
    clamp: Filtered image pixels will be clamped to the following
        range. Default: -infinity..infinity (i.e. no clamping is used))doc";
...
R"doc(Split an multi-channel image buffer (e.g. from an OpenEXR image
with lots of AOVs) into its constituent layers)doc";

static const char *__doc_mitsuba_Bitmap_srgb_gamma = R"doc(Return whether the bitmap uses an sRGB gamma encoding)doc";

static const char *__doc_mitsuba_Bitmap_static_initialization = R"doc(Static initialization of bitmap-related data structures (thread pools, etc.))doc";

static const char *__doc_mitsuba_Bitmap_static_shutdown = R"doc(Free the resources used by static_initialization())doc";

static const char *__doc_mitsuba_Bitmap_struct = R"doc(Return a `Struct` instance describing the contents of the bitmap (const version))doc";
...
R"doc(Write an encoded form of the bitmap to a stream using the specified file format
...
Args:
    ...
    quality: Depending on the file format, this parameter takes on a slightly
        ...
        * OpenEXR images: denotes the quality level of the DWAB
          ...
          to the lossless PIZ compressor.)doc";

static const char *__doc_mitsuba_Bitmap_write_2 =
R"doc(Write an encoded form of the bitmap to a file using the specified file format
...
static const char *__doc_mitsuba_Bitmap_write_async = R"doc(Equivalent to `write()`, but executes asynchronously on a different thread)doc";
...
Template Args:
    T: The underlying point data type (e.g. ``Point2d``))doc";
...
static const char *__doc_mitsuba_BoundingBox_BoundingBox_4 = R"doc(Create a bounding box from a smaller type (e.g. vectorized from scalar).)doc";
...
static const char *__doc_mitsuba_BoundingBox_collapsed = R"doc(Check whether this bounding box has collapsed to a point, line, or plane)doc";
...
Note:
    ...
    function parameter with default value ``False``.)doc";

static const char *__doc_mitsuba_BoundingBox_contains_2 =
R"doc(Check whether a specified bounding box lies *on* or *within*
...
R"doc(Calculate the shortest distance between
the axis-aligned bounding box and the point ``p``.)doc";

static const char *__doc_mitsuba_BoundingBox_distance_2 =
R"doc(Calculate the shortest distance between
the axis-aligned bounding box and ``bbox``.)doc";
...
static const char *__doc_mitsuba_BoundingBox_fields = R"doc()doc";

static const char *__doc_mitsuba_BoundingBox_fields_2 = R"doc()doc";

static const char *__doc_mitsuba_BoundingBox_labels = R"doc()doc";
...
static const char *__doc_mitsuba_BoundingBox_name = R"doc()doc";
...
R"doc(Calculate the shortest squared distance between
the axis-aligned bounding box and the point ``p``.)doc";
...
static const char *__doc_mitsuba_BoundingSphere_BoundingSphere_2 = R"doc(Create bounding sphere(s) from given center point(s) with given size(s))doc";
...
each wavelength conditions the underlying distribution.)doc";

static const char *__doc_mitsuba_ConditionalIrregular1D_ConditionalIrregular1D =
...
Args:
    ...
    sizes_cond: Array with the sizes of the conditional nodes arrays.)doc";

static const char *__doc_mitsuba_ConditionalIrregular1D_ConditionalIrregular1D_4 = R"doc()doc";
...
Returns:
    The value of the PDF at position ``x``, conditioned on ``cond``.)doc";

static const char *__doc_mitsuba_ConditionalIrregular1D_eval_pdf_normalized =
R"doc(Evaluate the normalized probability density function (PDF) at
position ``x``, conditioned on ``cond``.
...
R"doc(Sample the distribution given a uniform sample ``u``, conditioned
on ``cond``.
...
static const char *__doc_mitsuba_ConditionalRegular1D_ConditionalRegular1D =
...
Args:
    ...
    size_cond: Array with the size of each conditional dimension.)doc";
...
Args:
    ...
    range_cond: Array of ranges where the dimensional conditionals are defined.)doc";
...
Args:
    ...
    cond: Conditionals where the PDF is evaluated.)doc";
...
static const char *__doc_mitsuba_ContinuousDistribution_cdf = R"doc(Return the unnormalized discrete cumulative distribution function over intervals)doc";

static const char *__doc_mitsuba_ContinuousDistribution_cdf_2 = R"doc(Return the unnormalized discrete cumulative distribution function over intervals (const version))doc";
...
static const char *__doc_mitsuba_ContinuousDistribution_eval_cdf = R"doc(Evaluate the unnormalized cumulative distribution function (CDF) at position ``p``)doc";

static const char *__doc_mitsuba_ContinuousDistribution_eval_cdf_normalized = R"doc(Evaluate the unnormalized cumulative distribution function (CDF) at position ``p``)doc";

static const char *__doc_mitsuba_ContinuousDistribution_eval_pdf = R"doc(Evaluate the unnormalized probability density function (PDF) at position ``x``)doc";

static const char *__doc_mitsuba_ContinuousDistribution_eval_pdf_normalized = R"doc(Evaluate the normalized probability density function (PDF) at position ``x``)doc";
...
static const char *__doc_mitsuba_ContinuousDistribution_normalization = R"doc(Return the normalization factor (i.e. the inverse of `integral()`))doc";
...
static const char *__doc_mitsuba_ContinuousDistribution_pdf_2 = R"doc(Return the unnormalized discretized probability density function (const version))doc";
...
R"doc(Transform a uniformly distributed sample to the stored
distribution
...
Returns:
    ...
    1. the sampled position.
    2. the normalized probability density of the sample.)doc";
...
``UINT32_MAX`` marks a missing entry that resolves to zeros.)doc";
...
corner ``c`` reads at ``corner_index[c]``, instead of one per corner.)doc";
...
and end at ``corner_count``. Null input implies pure triangles.)doc";
...
R"doc(Calculate difference in days between the current Julian Day
...
R"doc(The default formatter used to turn log messages into
a human-readable form)doc";
...
Args:
    ...
    warn_defects: Report non-manifold or inconsistently wound input in a log message.
        ...
        which callers building the structure implicitly may wish to avoid.)doc";
...
point of the vertex walk described in the class documentation.)doc";
...
the sampled point.)doc";

static const char *__doc_mitsuba_DirectionSample_2 = R"doc()doc";

static const char *__doc_mitsuba_DirectionSample_DirectionSample =
R"doc(Create a direct sampling record, which can be used to *query*
...
static const char *__doc_mitsuba_DiscontinuityFlags_DirectionLune =
R"doc(Use spherical lune to encode segment direction

This flag is only relevant for certain shape types.)doc";

static const char *__doc_mitsuba_DiscontinuityFlags_DirectionSphere =
R"doc(Use spherical coordinates to encode segment direction

This flag is only relevant for certain shape types.)doc";

static const char *__doc_mitsuba_DiscontinuityFlags_Empty = R"doc(No flags set (default value))doc";

static const char *__doc_mitsuba_DiscontinuityFlags_HeuristicWalk =
R"doc(Project to an edge using a heuristic probability
...
R"doc(Generates samples proportional to a *discrete* 2D function sampled on
a regular grid
...
static const char *__doc_mitsuba_DiscreteDistribution2D_DiscreteDistribution2D =
R"doc(Construct a marginal sample warping scheme for floating point
data of resolution ``size``.)doc";

static const char *__doc_mitsuba_DiscreteDistribution2D_DiscreteDistribution2D_2 = R"doc()doc";
...
static const char *__doc_mitsuba_DiscreteDistribution_cdf_2 = R"doc(Return the unnormalized cumulative distribution function (const version))doc";
...
static const char *__doc_mitsuba_DiscreteDistribution_eval_cdf = R"doc(Evaluate the unnormalized cumulative distribution function (CDF) at index ``index``)doc";

static const char *__doc_mitsuba_DiscreteDistribution_eval_cdf_normalized = R"doc(Evaluate the normalized cumulative distribution function (CDF) at index ``index``)doc";

static const char *__doc_mitsuba_DiscreteDistribution_eval_pmf = R"doc(Evaluate the unnormalized probability mass function (PMF) at index ``index``)doc";

static const char *__doc_mitsuba_DiscreteDistribution_eval_pmf_normalized = R"doc(Evaluate the normalized probability mass function (PMF) at index ``index``)doc";
...
Returns:
    ...
    2. the normalized probability value of the sample.)doc";
...
Returns:
    ...
    1. the discrete index associated with the sample, and
    2. the re-scaled sample value.)doc";
...
Returns:
    ...
    3. the normalized probability value of the sample)doc";
...
to store serialized content.)doc";
...
It may be called automatically by the destructor.)doc";
...
R"doc(Current position in the "virtual" stream (even though nothing
is ever written, we need to maintain consistent positioning).)doc";

static const char *__doc_mitsuba_DummyStream_m_size = R"doc(Size of all data written to the stream)doc";

static const char *__doc_mitsuba_DummyStream_read = R"doc()doc";
...
these redundancies and remove them retroactively.)doc";
...
static const char *__doc_mitsuba_Endpoint_bbox =
R"doc(Return an axis-aligned box bounding the spatial
extents of the emitter)doc";

static const char *__doc_mitsuba_Endpoint_class_name = R"doc()doc";

static const char *__doc_mitsuba_Endpoint_eval =
R"doc(Given a ray-surface intersection, return the emitted
...
Returns:
    The incident direct radiance/importance associated with the sample.)doc";
...
static const char *__doc_mitsuba_Endpoint_medium_2 = R"doc(Return a pointer to the `Medium` that surrounds the emitter (const version))doc";

static const char *__doc_mitsuba_Endpoint_needs_sample_2 =
R"doc(Does the method `sample_ray()` require a uniformly distributed
2D sample for the ``sample2`` parameter?)doc";

static const char *__doc_mitsuba_Endpoint_needs_sample_3 =
R"doc(Does the method `sample_ray()` require a uniformly distributed
2D sample for the ``sample3`` parameter?)doc";
...
R"doc(Evaluate the probability density of the *direct* sampling
...
R"doc(Evaluate the probability density of the position sampling
...
R"doc(Evaluate the probability density of the wavelength sampling
...
Returns:
    ...
    along with a spectral importance weight.)doc";

static const char *__doc_mitsuba_Endpoint_sample_position =
R"doc(Importance sample the spatial component of the
...
Returns:
    ...
    along with an importance weight.)doc";
...
R"doc(Importance sample a set of wavelengths according to the
...
Returns:
    ...
    emitters, the weight will include the emitted radiance.)doc";
...
This function is invoked by the `Scene` constructor.)doc";
...
static const char *__doc_mitsuba_Endpoint_shape_2 = R"doc(Return the `Shape` to which the emitter is currently attached (const version))doc";
...
search paths in order and stops once the file is found.)doc";
...
Raises:
    RuntimeError: If the file cannot be opened / created, or if write
        is disabled and the file does not already exist.)doc";

static const char *__doc_mitsuba_FileStream_can_read = R"doc(True except if the stream was closed.)doc";

static const char *__doc_mitsuba_FileStream_can_write = R"doc(Whether the file was open in write-mode (and was not closed))doc";

static const char *__doc_mitsuba_FileStream_class_name = R"doc()doc";

static const char *__doc_mitsuba_FileStream_close =
R"doc(Closes the stream and the underlying file.
No further read or write operations are permitted.

This function is idempotent.
It is called automatically by the destructor.)doc";
...
Throws an exception when the stream ended prematurely.)doc";
...
static const char *__doc_mitsuba_FileStream_seek = R"doc(Seeks to a position inside the stream. May throw if the resulting state is invalid.)doc";
...
Note:
    ...
    until a `flush()` is performed.)doc";
...
R"doc(Truncates the file to a given size.
...
R"doc(Abstract film base class - used to store samples
generated by `Integrator` implementations.
...
is then committed to the film using the `ImageBlock.put()` method.)doc";
...
R"doc(The film provides a customized `Film.prepare_sample()` routine that implements
a special treatment of the samples before storing them in the `ImageBlock`.)doc";
...
R"doc(Return an `ImageBlock` instance, whose internal representation
...
Args:
    ...
    borders: Should `ImageBlock` add an additional border region around
        the image boundary? See the `ImageBlock` constructor for details.)doc";
...
Returns the total number of channels that the film will store)doc";
...
Args:
    ...
    channel_count: Number of spectral sensitivities of the film

    weight: Value to be added to the weight channel of the sample

    alpha: Alpha value of the sample

    active: Mask indicating if the lanes are active

Returns:
    An array of length ``channel_count`` specifying the sample value
    for each channel.)doc";

static const char *__doc_mitsuba_Film_put_block = R"doc(Merge an image block into the film. This methods should be thread-safe.)doc";
...
sense when reconstruction filters other than the box filter are used.)doc";
...
Args:
    ...
    crop_size: The size of the crop window.)doc";
...
Args:
    size: The new size of the film.)doc";

static const char *__doc_mitsuba_Film_size = R"doc(Ignoring the crop window, return the resolution of the underlying sensor)doc";
...
static const char *__doc_mitsuba_FilterBoundaryCondition_One = R"doc(Assume that the input function is equal to one outside of the defined domain)doc";
...
R"doc(Abstract interface for converting log information into
a human-readable format)doc";
...
Args:
    ...
    msg: Text content associated with the log message)doc";
...
coordinate system whose pole is aligned with the ``n`` axis (e.g. `cos_theta()`, `sin_phi()`, etc.).)doc";
...
R"doc(Give a unit direction, this function returns the squared cosine of
the azimuth in a reference spherical coordinate system (see the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_cos_theta =
R"doc(Give a unit direction, this function returns the cosine of the
elevation angle in a reference spherical coordinate system (see the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_cos_theta_2 =
R"doc(Give a unit direction, this function returns the square cosine
of the elevation angle in a reference spherical coordinate system (see
the `Frame3f` description))doc";
...
R"doc(Give a unit direction, this function returns the sine of the
azimuth in a reference spherical coordinate system (see the `Frame3f`
description))doc";

static const char *__doc_mitsuba_Frame_sin_phi_2 =
R"doc(Give a unit direction, this function returns the squared sine of
the azimuth in a reference spherical coordinate system (see the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_sin_theta =
R"doc(Give a unit direction, this function returns the sine
of the elevation angle in a reference spherical coordinate system (see
the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_sin_theta_2 =
R"doc(Give a unit direction, this function returns the square sine
of the elevation angle in a reference spherical coordinate system (see
the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_sincos_phi =
R"doc(Give a unit direction, this function returns the sine and cosine
of the azimuth in a reference spherical coordinate system (see the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_sincos_phi_2 =
R"doc(Give a unit direction, this function returns the squared sine
and cosine of the azimuth in a reference spherical coordinate system
(see the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_t = R"doc()doc";

static const char *__doc_mitsuba_Frame_tan_theta =
R"doc(Give a unit direction, this function returns the tangent
of the elevation angle in a reference spherical coordinate system (see
the `Frame3f` description))doc";

static const char *__doc_mitsuba_Frame_tan_theta_2 =
R"doc(Give a unit direction, this function returns the square tangent
of the elevation angle in a reference spherical coordinate system (see
the `Frame3f` description))doc";
...
FIRST sample therefore lies exactly on the corner :math:`(0, 0)` and the LAST
on :math:`(1, 1)`, and the array spans ``res - 1`` interpolation patches
rather than ``res`` cells. Data that is conceptually PIXEL-CENTRED (an image,
a histogram, a point spread function) differs from this by half a cell, which
matters most where the density is concentrated: e.g. a radially symmetric
pattern whose peak must map to :math:`(0.5, 0.5)` requires an ODD ``res``,
since for even ``res`` the central sample sits at ``(res / 2) / (res - 1)``,
half a cell past the centre.
...
via the ``Dimension`` template parameter).

In this case, the input array should have dimensions ``N0 x N1 x ... x Nn
x res.y() x res.x()`` (where the last dimension is contiguous in memory),
and the ``param_res`` should be set to ``{ N0, N1, ..., Nn }``,
and ``param_values`` should contain the parameter values where the
...
Note:
    ...
    on 0 to 3 parameters.)doc";

static const char *__doc_mitsuba_Hierarchical2D_Hierarchical2D =
R"doc(Construct a hierarchical sample warping scheme for floating point
data of resolution ``size``.

``param_res`` and ``param_values`` are only needed for conditional
distributions (see the text describing the Hierarchical2D class).

If ``normalize`` is set to ``False``, the implementation will not
re-scale the distribution so that it integrates to ``1``. It can
still be sampled (proportionally), but returned density values
will reflect the unnormalized values.

If ``enable_sampling`` is set to ``False``, the implementation will not
...
will not return meaningful results.)doc";

static const char *__doc_mitsuba_Hierarchical2D_Hierarchical2D_2 = R"doc()doc";
...
R"doc(Convert from 2D pixel coordinates to an index indicating how the
...
static const char *__doc_mitsuba_Hierarchical2D_m_data = R"doc(Unified storage buffer: level 0 (row-major) followed by the MIP hierarchy)doc";
...
evaluate the reverse-mode derivative of the `put()` method.)doc";
...
Args:
    ...
    warn_invalid: If set to ``True``, `put()` will warn when writing samples with
        ...
        checking/error reporting is relatively costly in JIT-compiled modes.)doc";
...
R"doc(Accumulate a single sample or a wavefront of samples into the
image block.
...
Args:
    ...
    values: Points to an array of length `channel_count()`, which specifies
        the sample value for each channel.)doc";
...
R"doc(Fetch a single sample or a wavefront of samples from the image
block.
...
Args:
    ...
    values: Points to an array of length `channel_count()`, which will
        ...
        Python, the function returns these values as a list.)doc";
...
static const char *__doc_mitsuba_InstanceEntry = R"doc(One flattened TLAS/IAS instance: a transformed reference to a ``BlasEntry``.)doc";
...
Args:
    ...
    spp: Optional parameter to override the number of samples per pixel for the
        ...
        scene specification takes precedence if ``spp=0``.)doc";
...
static const char *__doc_mitsuba_Interaction = R"doc(Generic interaction data structure, shared by surface and medium interactions)doc";

static const char *__doc_mitsuba_Interaction_2 = R"doc()doc";
...
static const char *__doc_mitsuba_Interaction_n = R"doc(Geometric normal (only valid for `SurfaceInteraction3f`))doc";
...
R"doc(This callback method is invoked by dr::zeros<>, and takes care of fields that deviate
from the standard zero-initialization convention. In this particular class, the ``t``
field should be set to an infinite value to mark invalid interaction records.)doc";
...
static const char *__doc_mitsuba_IrregularContinuousDistribution_IrregularContinuousDistribution_2 = R"doc(Initialize from a given density function discretized on nodes ``nodes``)doc";
...
static const char *__doc_mitsuba_IrregularContinuousDistribution_cdf = R"doc(Return the unnormalized discrete cumulative distribution function over intervals)doc";

static const char *__doc_mitsuba_IrregularContinuousDistribution_cdf_2 = R"doc(Return the unnormalized discrete cumulative distribution function over intervals (const version))doc";
...
static const char *__doc_mitsuba_IrregularContinuousDistribution_eval_cdf = R"doc(Evaluate the unnormalized cumulative distribution function (CDF) at position ``p``)doc";

static const char *__doc_mitsuba_IrregularContinuousDistribution_eval_cdf_normalized = R"doc(Evaluate the unnormalized cumulative distribution function (CDF) at position ``p``)doc";

static const char *__doc_mitsuba_IrregularContinuousDistribution_eval_pdf = R"doc(Evaluate the unnormalized probability density function (PDF) at position ``x``)doc";

static const char *__doc_mitsuba_IrregularContinuousDistribution_eval_pdf_normalized = R"doc(Evaluate the normalized probability density function (PDF) at position ``x``)doc";
...
static const char *__doc_mitsuba_IrregularContinuousDistribution_normalization = R"doc(Return the normalization factor (i.e. the inverse of `integral()`))doc";
...
static const char *__doc_mitsuba_IrregularContinuousDistribution_pdf_2 = R"doc(Return the unnormalized discretized probability density function (const version))doc";
...
The ``MI_DECLARE_PLUGIN_BASE_CLASS()`` macro ensures that these attributes are present.)doc";
...
R"doc(Implements a marginal sample warping scheme for 2D distributions
with linear interpolation and an optional dependence on additional parameters
...
position inside the patch. When ``Continuous=true``,
...
static const char *__doc_mitsuba_Marginal2D_Marginal2D =
R"doc(Construct a marginal sample warping scheme for floating point
data of resolution ``size``.
...
static const char *__doc_mitsuba_Marginal2D_Marginal2D_2 = R"doc()doc";
...
R"doc(This callback method is invoked by dr::zeros<>, and takes care of fields that deviate
from the standard zero-initialization convention.)doc";
...
R"doc(Returns the medium coefficients :math:`\sigma_s`, :math:`\sigma_n` and
:math:`\sigma_t` evaluated at a given `MediumInteraction3f` ``mi``)doc";
...
generally change the pointer obtained via `data()`)doc";
...
as data is written to the stream, à la ``std::vector``.)doc";

static const char *__doc_mitsuba_MemoryStream_MemoryStream =
R"doc(Creates a new memory stream, initializing the memory buffer
with a capacity of ``capacity`` bytes. For best performance,
set this argument to the estimated size of the content that
will be written to the stream.)doc";
...
buffer's capacity. The size and capacity are **not** affected.
A subsequent write would then expand the size and capacity
accordingly. The contents of the memory that was skipped is undefined.)doc";

static const char *__doc_mitsuba_MemoryStream_size =
R"doc(Returns the size of the contents written to the memory buffer.

Note:
    This is not equal to the size of the memory buffer in general,
    since we allocate more capacity at once.)doc";

static const char *__doc_mitsuba_MemoryStream_tell =
R"doc(Gets the current position inside the memory buffer. Note that
this might be further than the stream's size or even capacity.)doc";
...
R"doc(Truncates the contents **and** the memory buffer's capacity
to a given size.
The position is updated to ``min(old_position, size)``.

Note:
    This will throw is the MemoryStream was initialized with a
    pre-allocated buffer.)doc";

static const char *__doc_mitsuba_MemoryStream_write =
R"doc(Writes a specified amount of data into the memory buffer.
The capacity of the memory buffer is extended if necessary.)doc";
...
``to_world``, and subsequent transformations via `transform()`.)doc";
...
tensor ``values``. Attributes may carry 1 to 4 dimensions.)doc";
...
R"doc(Compute smooth shading normals from the field views, accumulated
over the ``m_normal_index`` grouping, as an ``(N, 3)`` tensor)doc";
...
R"doc(Compute MikkTSpace shading tangents from the field views as a
``(V, 3)`` tensor)doc";
...
variants), where a single thread orchestrates the parallel computation.)doc";
...
R"doc(Returns the opposite edge index associated with directed edge ``index``
...
the absence of a neighboring face.)doc";
...
(The ``PackedMesh`` class provides an API for this.))doc";
...
static const char *__doc_mitsuba_Mesh_holds_rgb2spec_coeffs = R"doc(Do the records of the attribute ``name`` hold RGB2Spec upsampling coefficients?)doc";
...
static const char *__doc_mitsuba_Mesh_is_vertex_attribute = R"doc(Does the attribute ``name`` live on the vertices rather than the faces?)doc";
...
R"doc(Custom mesh attributes. The use of a node-based map is intentional
as this provides stable references.)doc";
...
Returns:
    ...
    - ``mask`` indicates whether an intersection was found.)doc";
...
the layout that the next repack will write, see `packs_tangent()`.)doc";
...
R"doc(Return the vertex index -> normal index map. An empty map
...
R"doc(Return the vertex index -> surface position index map. An empty
map encodes the identity.)doc";
...
Returns:
    ...
    - ``uv`` contains the first two barycentric coordinates.)doc";
...
R"doc(Return the sampling density over edges that could contribute to
...
static const char *__doc_mitsuba_Mesh_to_rgb2spec_coeffs = R"doc(Convert ``rows`` RGB triplets in place into RGB2Spec upsampling coefficients.)doc";
...
derivatives propagate from ``t`` to the resulting mesh state.)doc";
...
because it requires several device reductions.)doc";
...
rewrite the offending field.)doc";
...
R"doc(Returns the world-space position of the vertex with index
``index``
...
variants of `ray_intersect_triangle()` pass a ``dr::Packet``.)doc";
...
static const char *__doc_mitsuba_MetalAccelData = R"doc(Opaque handle owning the native Metal objects, see src/render/metal_accel.mm)doc";
...
static const char *__doc_mitsuba_MiOptixSceneState = R"doc(Per-scene OptiX state (SBT/GAS/IAS), defined in src/render/scene_optix.inl)doc";

static const char *__doc_mitsuba_MicrofacetDistribution =
R"doc(Implementation of the Beckmann and GGX / Trowbridge-Reitz microfacet
...
and GGX models, respectively.)doc";
...
Args:
    ...
    alpha: The surface roughness)doc";
...
Args:
    ...
    alpha_v: The surface roughness in the bitangent direction)doc";

static const char *__doc_mitsuba_MicrofacetDistribution_MicrofacetDistribution_3 =
R"doc(Create a microfacet distribution from a `Properties` data
structure)doc";
...
R"doc(Returns the density function associated with
the `sample()` function.
...
R"doc(Draw a sample from the microfacet normal distribution
...
Returns:
    ...
    and the associated solid angle density)doc";
...
must detect and apply changes when executing frozen functions.)doc";
...
.. note::
   ...
   its usage in macros that appear before the full definition.)doc";
...
variance as a feature of the scene and will not denoise it.)doc";
...
Args:
    input_size: Resolution of noisy images that will be fed to the denoiser.

    albedo: Whether or not albedo information will also be given to the
        denoiser.
        This parameter is optional, by default it is false.

    normals: Whether or not shading normals information will also be given to the
        denoiser.
        This parameter is optional, by default it is false.

    temporal: Whether or not temporal information will also be given to the
        denoiser.
        This parameter is optional, by default it is false.

    denoise_alpha: Whether or not the alpha channel (if specified in the noisy input)
...
Args:
    noisy: The noisy input. (tensor shape: (width, height, 3 | 4))

    albedo: Albedo information of the noisy rendering.
        This parameter is optional unless the OptixDenoiser was built with
        albedo support. (tensor shape: (width, height, 3))

    normals: Shading normal information of the noisy rendering. The normals must
        be in the coordinate frame of the sensor which was used to render
        the noisy input.
        This parameter is optional unless the OptixDenoiser was built with
        normals support. (tensor shape: (width, height, 3))

    to_sensor: An `AffineTransform4f` which is applied to the ``normals`` parameter
    ...
    flow: With temporal denoising, this parameter is the optical flow between
    ...
    previous_denoised: With temporal denoising, the previous denoised frame should be
        passed here.
...
Args:
    ...
    normals_ch: The name of the channel in the ``noisy`` parameter which contains
    ...
    flow_ch: With temporal denoising, this parameter is name of the channel in
    ...
    previous_denoised_ch: With temporal denoising, this parameter is name of the channel in
        the ``noisy`` parameter which contains the previous denoised frame.
        For the very first frame, the OptiX documentation recommends
        passing the noisy input for this argument.
        This parameter is optional unless the OptixDenoiser was built with
        temporal denoising support.

    noisy_ch: The name of the channel in the ``noisy`` parameter which contains
...
static const char *__doc_mitsuba_PCG32Sampler = R"doc(Interface for sampler plugins based on the PCG32 random number generator)doc";
...
static const char *__doc_mitsuba_PackedMesh_face_count = R"doc(Sizes of the vertex/face/position/normal arrays)doc";
...
introduce discontinuities in the Monte Carlo simulation.)doc";

static const char *__doc_mitsuba_ParamFlags_Differentiable = R"doc(Tracking gradients w.r.t. this parameter is allowed)doc";

static const char *__doc_mitsuba_ParamFlags_Discontinuous = R"doc(Tracking gradients w.r.t. this parameter will introduce discontinuities)doc";
...
Mitsuba. It exposes functions for evaluating and sampling the model.)doc";
...
static const char *__doc_mitsuba_PhaseFunctionContext_component =
R"doc(Integer value of requested phase function component index to be
...
static const char *__doc_mitsuba_PhaseFunctionContext_type_mask =
R"doc(Bit mask for requested phase function component types to be
sampled/evaluated.
The default value (equal to `PhaseFunctionFlags.Isotropic` |
`PhaseFunctionFlags.Anisotropic` | `PhaseFunctionFlags.Microflake`)
enables all components.)doc";

static const char *__doc_mitsuba_PhaseFunctionFlags =
R"doc(This enumeration is used to classify phase functions into different types,
...
if the phase function is not a microflake phase function.)doc";
...
Returns:
    The value and the sampling PDF of the phase function in direction ``wo``)doc";
...
that they can be instantiated analogously.)doc";
...
Args:
    props: A `Properties` instance containing all information required to
        find and construct the plugin.)doc";
...
type from the provided class ``T``.)doc";
...
Args:
    ...
    payload: An opaque pointer parameter that will be forwarded
        to both ``instantiate`` and ``release``.)doc";
...
probability density and delta.)doc";

static const char *__doc_mitsuba_PositionSample_2 = R"doc()doc";
...
static const char *__doc_mitsuba_PositionSample_delta = R"doc(Set if the sample was drawn from a degenerate (Dirac delta) distribution)doc";
...
``valid`` and sets ``t`` to infinity for invalid intersection records.)doc";

static const char *__doc_mitsuba_Profiler = R"doc()doc";

static const char *__doc_mitsuba_ProfilerPhase =
R"doc(List of 'phases' that are handled by the profiler. Note that a partial order
is assumed -- if a method "B" can occur in a call graph of another method
"A", then "B" must occur after "A" in the list below.)doc";
...
Mitsuba is run on the console, via the OpenGL GUI, or in Jupyter Notebook.)doc";
...
Args:
    ...
    payload: Custom pointer payload to be delivered as part of progress messages)doc";
...
static const char *__doc_mitsuba_ProgressReporter_update = R"doc(Update the progress to ``progress`` (which should be in the range [0, 1]))doc";
...
R"doc(Temporary data structure to store spectral data before expansion into a
plugin like ``RegularSpectrum`` or ``IrregularSpectrum``.)doc";
...
static const char *__doc_mitsuba_Properties_Spectrum_Spectrum_4 = R"doc(Construct spectrum from string (either single value or wavelength:value pairs))doc";
...
static const char *__doc_mitsuba_Properties_Spectrum_Spectrum_7 = R"doc(Construct irregular spectrum from separate wavelength and value strings)doc";
...
static const char *__doc_mitsuba_Properties_as_string_2 = R"doc(Return one of the parameters (converting it to a string if necessary, with default value))doc";
...
to obtain vectorized (e.g. JIT-compiled) arrays.)doc";
...
during scene optimization.)doc";

static const char *__doc_mitsuba_Properties_id =
R"doc(Returns a unique identifier associated with this instance (or an empty string)
...
as accessed only after successful type casting.)doc";
...
it creates a new entry with the given name and an empty value.)doc";
...
``props`` if they have the same name.)doc";
...
Returns:
    ``True`` upon success, ``False`` if the old property doesn't exist or new name already exists)doc";
...
Any container and can be retrieved later using ``get_any<T>()``.)doc";
...
Args:
    ...
    scramble: Selects the desired permutation type, where ``-1`` denotes the
        ...
        to be built seeded by the value of ``scramble``.)doc";
...
function implementation.)doc";

static const char *__doc_mitsuba_RadicalInverse_bases = R"doc(Return the number of prime bases for which precomputed tables are available)doc";
...
is run through an extra scrambling permutation.)doc";

static const char *__doc_mitsuba_RadicalInverse_inverse_permutation = R"doc(Return the inverse permutation corresponding to the given prime number basis)doc";
...
R"doc(Ray differential -- enhances the basic ray class with
offset rays for two adjacent pixels on the view plane)doc";
...
R"doc(Ignore the differentiable dependence of the
`SurfaceInteraction3f` with respect to shape parameters.

With `RayFlags.DetachShape`, the shape's parameters are detached before the
...
R"doc(Track differentiable dependence of the `SurfaceInteraction3f`
...
With `RayFlags.FollowShape`, the intersection is instead rigidly glued to the
...
R"doc(Additionally compute normal partial derivatives
(`SurfaceInteraction3f.dn_du`, `SurfaceInteraction3f.dn_dv`), which
encode information about curvature.

Depends on `RayFlags.Shading`.)doc";

static const char *__doc_mitsuba_RayFlags_Shading =
R"doc(Additionally compute the UV coordinates
(`SurfaceInteraction3f.uv`), position partials
(`SurfaceInteraction3f.dp_du`, `SurfaceInteraction3f.dp_dv`),
...
representation, whose resolution given by ``MI_FILTER_RESOLUTION``.)doc";
...
static const char *__doc_mitsuba_ReconstructionFilter_eval_discretized = R"doc(Evaluate a discretized version of the filter (generally faster than `eval()`))doc";
...
R"doc(Utility class for efficiently resampling discrete datasets to different resolutions
...
R"doc(Create a new Resampler object that transforms between the specified resolutions
...
Args:
    ...
    target_res: Desired target resolution)doc";

static const char *__doc_mitsuba_Resampler_boundary_condition =
R"doc(Return the boundary condition that should be used when
looking up samples outside of the defined input domain)doc";
...
R"doc(Resample a multi-channel array and clamp the results
...
Args:
    ...
    channels: Number of channels to be resampled)doc";
...
R"doc(Set the boundary condition that should be used when
...
R"doc(The parameters of the SGGX phase function stored as a pair of
3D vectors [[S_xx, S_yy, S_zz], [S_xy, S_xz, S_yz]])doc";

static const char *__doc_mitsuba_SGGXPhaseFunctionParams_SGGXPhaseFunctionParams =
R"doc(Construct from a pair of 3D vectors [S_xx, S_yy, S_zz] and
...
must be called with ``wavefront_size`` matching the size of the wavefront.)doc";
...
Must be a multiple of the total sample count per pixel.
If set to (uint32_t) -1, all the work is done in a single pass (default).)doc";

static const char *__doc_mitsuba_SamplingIntegrator_render = R"doc()doc";
...
R"doc(Evaluate the discrete probability of the
`sample_emitter()` technique for the given a emitter index.)doc";
...
R"doc(Ray intersection using a brute force search. Used in
...
Returns:
    ...
    to the inverse PDF), and the transformed random sample for reuse.)doc";
...
Returns:
    ...
    * ``spec`` is a Monte Carlo sampling weight specifying the ratio
      ...
      probability per unit solid angle.)doc";
...
R"doc(Map a point sample in boundary sample space to a silhouette
segment
...
R"doc(Specifies whether any of the scene's shape parameters have
gradient tracking enabled)doc";
...
Returns:
    ...
    importance weights. The latter account for the difference between the
    sensor profile and the actual used sampling density function.)doc";
...
Returns:
    ...
    profile and the actual used sampling density function.)doc";
...
anything except creating forks.)doc";
...
`Shape.eval_attribute_1` and `Shape.eval_attribute_3` methods.)doc";
...
static const char *__doc_mitsuba_ShapeGroup_shape_types = R"doc(Returns a union of `ShapeType` flags denoting what is present in the ShapeGroup)doc";
...
R"doc(Mitsuba bundles each of the following geometry kinds into its own BLAS.
Instance must remain last (see ``NumGeometryKinds``).)doc";
...
R"doc(Map an abstract ``TShapeKDTree`` primitive index to a specific
...
create a detailed intersection record.)doc";
...
(including any transformations that may have been applied to them))doc";
...
`Shape.bbox` and clips it to ``clip``.)doc";
...
R"doc(Invalidate hits whose geometric normal faces along the ray, enforcing the
single-sided contract on backends (Metal) that report both faces.)doc";
...
R"doc(Return the number of primitives (triangles, hairs, ..)
...
R"doc(Monochromatic evaluation of a shape attribute at the given surface interaction
...
R"doc(Trichromatic evaluation of a shape attribute at the given surface interaction
...
R"doc(Evaluate a dynamically sized shape attribute at the given surface interaction.
...
R"doc(Query the probability density of `sample_position()` for
...
R"doc(Precompute the visible silhouette of this shape for a given
viewpoint.
...
R"doc(Projects a point on the surface of the shape to its silhouette
...
Args:
    ...
    ray_flags: Describe how the detailed information should be computed)doc";
...
Returns:
    ...
    ``ShapeGroup`` class and be set to ``(uint32_t)-1`` otherwise.)doc";
...
R"doc(Sample a direction towards this shape with respect to solid
...
Returns:
    A `DirectionSample3f` instance describing the generated sample)doc";
...
Returns:
    A `PositionSample3f` instance describing the generated sample)doc";

static const char *__doc_mitsuba_Shape_sample_precomputed_silhouette =
R"doc(Samples a boundary segment on the shape's silhouette using
...
static const char *__doc_mitsuba_Shape_silhouette_discontinuity_types = R"doc()doc";
...
static const char *__doc_mitsuba_SilhouetteSample_2 = R"doc()doc";
...
static const char *__doc_mitsuba_SilhouetteSample_flags = R"doc(The set of `DiscontinuityFlags` that were used to generate this sample)doc";
...
R"doc(Offset along the boundary segment direction (``d``) to avoid
self-intersections.)doc";
...
static const char *__doc_mitsuba_Spectrum = R"doc()doc";
...
static const char *__doc_mitsuba_Spiral_Spiral = R"doc(Create a new spiral generator for the given size, offset into a larger frame, and block size)doc";
...
static const char *__doc_mitsuba_Spiral_reset = R"doc(Reset the spiral to its initial state. Does not affect the number of passes.)doc";
...
R"doc(`Appender` implementation, which writes to an
arbitrary C++ output stream)doc";
...
to that of the system (i.e. no conversion is performed))doc";
...
static const char *__doc_mitsuba_Stream_needs_endianness_swap = R"doc(Returns true if we need to perform endianness swapping before writing or reading.)doc";
...
Raises:
    RuntimeError: If the stream ended prematurely.)doc";

static const char *__doc_mitsuba_Stream_read_2 =
R"doc(Reads one object of type T from the stream at the current position
...
R"doc(Reads multiple objects of type T from the stream at the current position
by delegating to the appropriate ``serialization_helper``.
...
Raises:
    RuntimeError: If in read-only mode.)doc";
...
Raises:
    RuntimeError: If not all data could be written.)doc";
...
R"doc(Return the bonus factor for empty space used by the
tree construction heuristic)doc";
...
R"doc(Initialize the surface area heuristic with the bounds of
a parent node
...
updates the AD state of ``t`` and ``p``, so that their derivatives express
...
further. Those parameterized by ``pi.prim_uv`` must furthermore update it
to match ``p`` by projecting the (primal-zero) displacement ``p -
p_att`` onto the tangent basis and attaching it via ``dr::replace_grad``.

Args:
    p_att: Surface position at the *detached* parameterization, attached to the
        shape's parameters. Its primal value equals ``p``.)doc";
...
Args:
    pi: Preliminary intersection which was used to during the call to
        `Shape.compute_surface_interaction()`
...
Args:
    M_world: The Mueller matrix in world space.

    in_forward_world: Incident direction (along propagation direction of light),
        given in world-space coordinates.

    out_forward_world: Outgoing direction (along propagation direction of light),
        given in world-space coordinates.

Returns:
    Equivalent Mueller matrix that operates in local frame coordinates.)doc";
...
Args:
    M_local: The Mueller matrix in local space, e.g. returned by a `BSDF`.

    in_forward_local: Incident direction (along propagation direction of light),
        given in local frame coordinates.

    out_forward_local: Outgoing direction (along propagation direction of light),
        given in local frame coordinates.

Returns:
    Equivalent Mueller matrix that operates in world-space coordinates.)doc";
...
R"doc(Optimized KD-tree acceleration data structure for n-dimensional
...
getters and setters of this class.)doc";

static const char *__doc_mitsuba_TShapeKDTree_BuildContext = R"doc(Helper data structure used during tree construction (shared by all threads))doc";

static const char *__doc_mitsuba_TShapeKDTree_BuildContext_2 = R"doc(Helper data structure used during tree construction (shared by all threads))doc";
...
static const char *__doc_mitsuba_TShapeKDTree_BuildTask_make_leaf = R"doc(Create a leaf node using the given set of indices (called by min-max binning))doc";

static const char *__doc_mitsuba_TShapeKDTree_BuildTask_make_leaf_2 = R"doc(Create a leaf node using the given edge event list (called by the O(N log N) builder))doc";

static const char *__doc_mitsuba_TShapeKDTree_BuildTask_transition_to_nlogn = R"doc(Create an initial sorted edge event list and start the O(N log N) builder)doc";

static const char *__doc_mitsuba_TShapeKDTree_BuildTask_traverse = R"doc(Traverse a subtree and collect all encountered primitive references in a set)doc";
...
std::vector<bool> specialization.)doc";
...
static const char *__doc_mitsuba_TShapeKDTree_KDNode_left_offset = R"doc(Assuming that this is an inner node, return the relative offset to the left child)doc";
...
static const char *__doc_mitsuba_TShapeKDTree_LocalBuildContext = R"doc(Helper data structure used during tree construction (used by a single thread))doc";
...
R"doc(Min-max binning data structure with parallel binning & partitioning steps

See
  "Highly Parallel Fast KD-tree Construction for Interactive Ray Tracing of
  Dynamic Scenes" by M. Shevtsov, A. Soupikov and A. Kapustin)doc";
...
R"doc(Given a suitable split candidate, compute tight bounding
...
static const char *__doc_mitsuba_TShapeKDTree_PrimClassification = R"doc(Enumeration representing the state of a classified primitive in the O(N log N) builder)doc";
...
R"doc(Return the number of primitives, at which the builder will switch
...
R"doc(Return the number of bad refines allowed to happen
in succession before a leaf node will be created.)doc";
...
R"doc(Specify the number of primitives, at which the builder will
...
R"doc(Set the number of bad refines allowed to happen
in succession before a leaf node will be created.)doc";
...
R"doc(Set the number of primitives, at which recursion will
stop when building the tree.)doc";

static const char *__doc_mitsuba_TShapeKDTree_stop_primitives =
R"doc(Return the number of primitives, at which recursion will
stop when building the tree.)doc";
...
upload the data and obtain a device tensor handle.)doc";
...
static const char *__doc_mitsuba_Transform_from_frame = R"doc(Creates a transformation that converts from ``frame`` to the standard basis)doc";
...
question and ``I`` is the identity).)doc";
...
static const char *__doc_mitsuba_Transform_matrix = R"doc()doc";
...
Note:
    In the Python API, this maps to the ``@`` operator)doc";
...
R"doc(Create an orthographic transformation, which maps Z to [0,1]
...
R"doc(Create a perspective transformation.
...
R"doc(Specifies the transport mode when sampling or
evaluating a scattering function)doc";
...
static const char *__doc_mitsuba_TraversalCallback_put_4 = R"doc()doc";

static const char *__doc_mitsuba_TraversalCallback_put_object = R"doc(Actual implementation for `Object` references [To be provided by subclass])doc";

static const char *__doc_mitsuba_TraversalCallback_put_value = R"doc(Actual implementation for regular parameters [To be provided by subclass])doc";

static const char *__doc_mitsuba_Vector = R"doc()doc";
...
Args:
    path: Name of the file to be loaded)doc";

static const char *__doc_mitsuba_VolumeGrid_VolumeGrid_2 =
R"doc(Load a VolumeGrid from an arbitrary stream data source

Args:
    stream: Pointer to an arbitrary stream data source)doc";

static const char *__doc_mitsuba_VolumeGrid_VolumeGrid_3 = R"doc()doc";

static const char *__doc_mitsuba_VolumeGrid_bbox_transform = R"doc(Estimates the transformation from a unit axis-aligned bounding box to the given one.)doc";

static const char *__doc_mitsuba_VolumeGrid_buffer_size = R"doc(Return the volume grid size in bytes (excluding metadata))doc";

static const char *__doc_mitsuba_VolumeGrid_bytes_per_voxel = R"doc(Return the number of bytes of storage used per voxel)doc";
...
Args:
    path: Target file name (expected to end in ".vol"))doc";
...
R"doc(Closes the stream, but not the underlying child stream.
No further read or write operations are permitted.
...
R"doc(Reads a specified amount of data from the stream, decompressing
it first using ZLib.
Throws an exception when the stream ended prematurely.)doc";
...
R"doc(Writes a specified amount of data into the stream, compressing
it first using ZLib.
Throws an exception when not all data could be written.)doc";
...
that can potentially live on the GPU and/or be differentiable.)doc";
...
nanometers, normalized to ensures that it integrates to a luminance of 1.0.)doc";
...
emitters where it is not clear how they should interact with polarization.)doc";
...
each thread will need its own allocator.)doc";
...
free memory. If no chunk could be found, a new one is created.)doc";
...
R"doc(The ``serialization_helper<T>`` implementations for new types should
in general be implemented as a series of calls to the lower-level
``serialization_helper::{read,write}`` functions.
This way, endianness swapping needs only be handled at the lowest level.)doc";
...
R"doc(Reads ``count`` values of type T from stream ``s``,
...
R"doc(Writes ``count`` values of type T into stream ``s``
starting at its current position.
Note: ``count`` is the number of values, **not** a size in bytes.
...
and the process-wide instance otherwise.)doc";
...
See Also:
    ...
    <http://en.cppreference.com/w/cpp/experimental/fs/absolute>`__)doc";
...
Creates intermediate directories if necessary.)doc";

static const char *__doc_mitsuba_filesystem_create_directory =
R"doc(Creates a directory at ``p`` as if ``mkdir`` was used.
...
does nothing (this condition is not treated as an error).)doc";
...
R"doc(Returns the size (in bytes) of a regular file at ``p``.
Attempting to determine the size of a directory (as well as any other file
that is not a regular file or a symlink) is treated as an error.)doc";

static const char *__doc_mitsuba_filesystem_is_directory = R"doc(Checks if ``p`` points to a directory.)doc";

static const char *__doc_mitsuba_filesystem_is_regular_file = R"doc(Checks if ``p`` points to a regular file, as opposed to a directory or symlink.)doc";

static const char *__doc_mitsuba_filesystem_path =
R"doc(Represents a path to a filesystem resource.
...
R"doc(Returns the path in the form of a native string, so that it can
...
R"doc(Assignment from the system's native string type. Acts similarly
to the string constructor.)doc";

static const char *__doc_mitsuba_filesystem_path_operator_basic_string =
R"doc(Implicit conversion operator to the basic_string corresponding
to the system's character type. Equivalent to calling ``native()``.)doc";
...
R"doc(Returns the path to the parent directory. Returns an empty path
if it is already empty or if it has only one element.)doc";

static const char *__doc_mitsuba_filesystem_path_path = R"doc(Default constructor. Constructs an empty path. An empty path is considered relative.)doc";
...
R"doc(Construct a path from a string view with native type.
On Windows, the path can use both '/' or '\\' as a delimiter.)doc";

static const char *__doc_mitsuba_filesystem_path_path_5 =
R"doc(Construct a path from a string with native type.
On Windows, the path can use both '/' or '\\' as a delimiter.)doc";

static const char *__doc_mitsuba_filesystem_path_path_6 =
R"doc(Construct a path from a native C-style string.
On Windows, this accepts wide strings (wchar_t*).
On other platforms, this accepts regular strings (char*).)doc";

static const char *__doc_mitsuba_filesystem_path_replace_extension =
R"doc(Replaces the substring starting at the rightmost ``.`` symbol
...
R"doc(Splits a string into tokens delimited by any of the characters
passed in ``delim``.)doc";
...
R"doc(Renames a file or directory. Returns true if renaming was
successful, false if there was an error (e.g. the file did not exist).)doc";
...
of the unit sphere decode to the same frame.)doc";

static const char *__doc_mitsuba_fresnel =
R"doc(Calculates the unpolarized Fresnel reflection coefficient
...
Note:
    ...
    assumption unless very thin layers are being simulated))doc";
...
Returns:
    ...
    - ``eta_ti`` is the reciprocal of the relative index of refraction in
      ...
      component of the refracted direction.)doc";
...
bits of the underlying floating point representation.)doc";

static const char *__doc_mitsuba_math_chi2 =
R"doc(Compute the :math:`\chi^2` statistic and degrees of freedom of the given
...
entries and resulting number of pooled regions.)doc";
...
static const char *__doc_mitsuba_math_legendre_pd = R"doc(Evaluate the l-th Legendre polynomial and its derivative using recurrence)doc";
...
R"doc(This function computes a suitable middle point for use in the ``bisect()`` function
...
In this case the function always returns 0.0 as the middle.)doc";
...
static const char *__doc_mitsuba_math_sample_shifted =
...
the context of Monte Carlo integration.)doc";

static const char *__doc_mitsuba_math_sample_shifted_2 = R"doc()doc";

static const char *__doc_mitsuba_math_solve_quadratic =
R"doc(Solve a quadratic equation of the form :math:`a x^2 + b x + c = 0`.
...
Returns:
    New Mueller matrix that operates from ``in_basis_target`` to ``out_basis_target``.)doc";
...
Returns:
    New Mueller matrix that operates from ``basis_target`` to ``basis_target``.)doc";
...
Returns:
    Mueller matrix that performs the desired change of reference frames.)doc";

static const char *__doc_mitsuba_mueller_rotated_element =
R"doc(Applies a counter-clockwise rotation to the mueller matrix
of a given element.)doc";
...
Args:
    ...
    eta: Complex-valued relative refractive index of the interface. In the real
        ...
        points into the region of lower density.)doc";
...
R"doc(Enable merging of identical plugin instances, e.g. materials
(default: true))doc";
...
R"doc(Node paths (e.g., "scene.myshape.mybsdf") parallel to ``nodes``.
Only used in the dictionary parser, specifically for error reporting.)doc";

static const char *__doc_mitsuba_parser_ParserState_nodes =
R"doc(The list of all scene nodes. The root node is at position 0.
Nodes are added during parsing and never removed, only modified)doc";
...
undergo further transformation before finally being instantiated.)doc";

static const char *__doc_mitsuba_parser_SceneNode_file_index =
R"doc(File index, identifies an entry of `ParserState.files`
Used for error reporting to show which file contains this node
(unused in the dict parser))doc";

static const char *__doc_mitsuba_parser_SceneNode_offset =
R"doc(Byte offset in the file where this node is found in the XML file
...
later resolved into concrete indices into ``ParserState.nodes``.)doc";

static const char *__doc_mitsuba_parser_SceneNode_type =
R"doc(Object type of this node (if known)
...
Args:
    ...
    state: Parser state to transform (modified in-place))doc";
...
Args:
    ...
    state: Parser state to optimize (modified in-place))doc";
...
Args:
    ...
    state: Parser state to modify in-place)doc";
...
Args:
    ...
    output_directory: Base directory where files should be relocated)doc";
...
Args:
    ...
    state: Parser state containing the scene to reorder (modified in-place))doc";
...
Raises:
    RuntimeError: If a reference cannot be resolved.)doc";
...
R"doc(PDF for the `sample_rgb_spectrum` strategy.
...
cases, the PDF is returned per wavelength.)doc";
...
Returns:
    The index corresponding to the input index in the pseudorandom permutation vector.)doc";
...
Args:
    n: Desired number of points)doc";

static const char *__doc_mitsuba_quad_composite_simpson =
R"doc(Computes the nodes and weights of a composite Simpson quadrature
...
R"doc(Computes the nodes and weights of a Gauss-Legendre quadrature
...
R"doc(Computes the nodes and weights of a Gauss-Lobatto quadrature
rule with the given number of evaluations.
...
tuple returned by the `fresnel` and `fresnel_polarized` functions.)doc";
...
static const char *__doc_mitsuba_sample_tea_float = R"doc(Alias to `sample_tea_float32` or `sample_tea_float64` based given type size)doc";
...
Returns:
    A uniformly distributed floating point number on the interval ``[0, 1)``)doc";
...
static const char *__doc_mitsuba_sample_wavelength = R"doc(Helper function to sample a wavelength (and a weight) given a random number)doc";
...
R"doc(Evaluates the probability of sampling a given normal
using the SGGX microflake distribution

Args:
    wm: The microflake normal

    s: The parameters of the SGGX phase function S_xx, S_yy, S_zz, S_xy, S_xz,
...
Returns:
    A normal (in world space) sampled from the distribution
    of visible normals)doc";
...
Args:
    filename: Path of the file to be read)doc";
...
Args:
    ...
    d65: Should the D65 illuminant be included in the integration. (default: False))doc";
...
Args:
    ...
    values: Array with the values to be stored in the file)doc";
...
Note:
    ...
    the function for many arguments ``x``.)doc";

static const char *__doc_mitsuba_spline_eval_1d_2 =
R"doc(Evaluate a cubic spline interpolant of a *non-uniformly* sampled 1D function
...
R"doc(Evaluate a cubic spline interpolant of a non-uniformly sampled 2D function
...
Note:
    ...
    inferred automatically from the size of the input arrays.)doc";

static const char *__doc_mitsuba_spline_eval_spline =
R"doc(Compute the value of a cubic spline that is parameterized by the
...
Returns:
    ...
    its derivative at ``t``)doc";

static const char *__doc_mitsuba_spline_eval_spline_i =
R"doc(Compute the definite integral and value of a cubic spline
...
Note:
    ...
    as the second and third elements of a triple.)doc";
...
R"doc(Invert a cubic spline interpolant of a *uniformly* sampled 1D function.
...
static const char *__doc_mitsuba_string_iequals = R"doc(Case-insensitive comparison of two string_views (warning: not unicode compliant))doc";

static const char *__doc_mitsuba_string_indent = R"doc(Indent every line of a string by some number of spaces)doc";

static const char *__doc_mitsuba_string_indent_2 = R"doc(Turn a type into a string representation and indent every line by some number of spaces)doc";
...
R"doc(Locale-independent string to floating point conversion analogous
...
the string following the parsed number contains non-whitespace characters.)doc";

static const char *__doc_mitsuba_string_strtof =
R"doc(Locale-independent string to floating point conversion analogous
to ``std::strtof``. (implemented using Daniel Lemire's fast_float library.)

On failure, ``endptr`` is set equal to ``s``.)doc";

static const char *__doc_mitsuba_string_to_lower = R"doc(Return a lower-case version of the given string (warning: not unicode compliant))doc";

static const char *__doc_mitsuba_string_to_upper = R"doc(Return a upper-case version of the given string (warning: not unicode compliant))doc";
...
static const char *__doc_mitsuba_type_suffix = R"doc(Convenience function which computes an array size/type suffix (like '2u' or '3fP'))doc";

static const char *__doc_mitsuba_unpolarized_spectrum =
R"doc(Return the (1,1) entry of a Mueller matrix. Identity function for all other types.
...
when writing a final RGB pixel value to the image block.)doc";
...
static const char *__doc_mitsuba_util_library_path = R"doc(Return the absolute path to ``libmitsuba-core.dylib/so/dll``)doc";
...
static const char *__doc_mitsuba_util_trap_debugger = R"doc(Generate a trap instruction if running in a debugger; otherwise, return.)doc";
...
static const char *__doc_mitsuba_warp_circ =
R"doc(Implements common warping techniques that map from the unit
...
distributed or weighted point sets in certain common target domains.)doc";
...
distribution matches the linear interpolant.)doc";

static const char *__doc_mitsuba_warp_interval_to_nonuniform_tent = R"doc(Warp a uniformly distributed sample on :math:`[0,1]` to a nonuniform tent distribution with nodes ``{a, b, c}``)doc";

static const char *__doc_mitsuba_warp_interval_to_tangent_direction = R"doc(Warp a uniformly distributed sample on :math:`[0,1]` to a direction in the tangent plane)doc";

static const char *__doc_mitsuba_warp_interval_to_tent = R"doc(Warp a uniformly distributed sample on :math:`[0,1]` to a tent distribution)doc";
...
static const char *__doc_mitsuba_warp_square_to_cosine_hemisphere = R"doc(Sample a cosine-weighted vector on the unit hemisphere with respect to solid angles)doc";
...
R"doc(Uniformly sample a vector that lies within a given
cone of angles around the Z axis

Args:
    cos_cutoff: Cosine of the cutoff angle

    sample: A uniformly distributed sample on :math:`[0,1]^2`)doc";

static const char *__doc_mitsuba_warp_square_to_uniform_cone_pdf =
R"doc(Density of `square_to_uniform_cone` with respect to solid angles.

Args:
    cos_cutoff: Cosine of the cutoff angle)doc";
...
static const char *__doc_mitsuba_warp_square_to_uniform_disk_concentric_pdf = R"doc(Density of `square_to_uniform_disk_concentric` per unit area)doc";
...
static const char *__doc_mitsuba_warp_square_to_uniform_hemisphere = R"doc(Uniformly sample a vector on the unit hemisphere with respect to solid angles)doc";
...
static const char *__doc_mitsuba_warp_square_to_uniform_sphere = R"doc(Uniformly sample a vector on the unit sphere with respect to solid angles)doc";
...
static const char *__doc_mitsuba_warp_square_to_uniform_triangle = R"doc(Convert an uniformly distributed square sample into barycentric coordinates)doc";
...
static const char *__doc_mitsuba_warp_square_to_von_mises_fisher = R"doc(Warp a uniformly distributed square sample to a von Mises Fisher distribution)doc";
...
static const char *__doc_mitsuba_warp_tangent_direction_to_interval = R"doc(Inverse of `interval_to_tangent_direction`)doc";

static const char *__doc_mitsuba_warp_tent_to_interval = R"doc(Warp a tent distribution to a uniformly distributed sample on :math:`[0,1]`)doc";
...
static const char *__doc_mitsuba_warp_von_mises_fisher_to_square = R"doc(Inverse of the mapping `square_to_von_mises_fisher`)doc";
