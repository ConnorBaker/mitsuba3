...
public:
    ...
    Color3f eval_3(const SurfaceInteraction3f &si,
        ...
        /* HSR: a single-channel texture BROADCASTS rather than throwing. `eval()` two
           ...
           wrapper plugin (`uv_wrapper`, `mix_color`, `blender_math`, ...) forwards
           `eval_3`, so a greyscale map behind any of them faulted mid-render. Broadcasting
           is also what every DCC means by a greyscale image in a colour slot:
           Blender/Cycles returns `make_float4(f, f, f, 1)` for IMAGE_DATA_TYPE_FLOAT /
           BYTE (intern/cycles/kernel/svm/image.h). */
        if (channels == 1)
