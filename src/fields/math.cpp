...
class MathTexture final : public SurfaceField<Float, Spectrum> {
public:
    MI_IMPORT_TYPES(Field, SurfaceField, Texture)

    MathTexture(const Properties &props) : SurfaceField(props) {
        for (const auto &prop : props.objects()) {
            if (Field *input = prop.try_get<Field>()) {
                if (!input->supports_surface_queries())
                    Throw("MathTexture: input \"%s\" does not support surface "
                          "queries", prop.name());
    ...
    FieldValueType out_type() const override {
        // A constant expression (no inputs) is scalar; otherwise the
        // expression is applied componentwise, so the output takes the
        // joint type of the inputs (scalars broadcast).
        if (m_inputs.empty())
            return FieldValueType::Float;

        bool all_scalar = true, any_color = false, any_array = false;
        for (const auto &input : m_inputs) {
            FieldValueType t = input->out_type();
            if (!(t == FieldValueType::Float && input->out_dim() == 1))
                all_scalar = false;
            any_color |= t == FieldValueType::Color3;
            any_array |= t == FieldValueType::Array3;
        }
