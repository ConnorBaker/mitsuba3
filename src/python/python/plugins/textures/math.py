...
# Registered as `blender_math` since the master port: upstream's C++ expression
# texture (src/fields/math.cpp, mitsuba3#1944) owns the plain name `math` now,
# and a Python plugin under a C++-owned name loses the registry race silently
# (see RESERVED_BY_CPP in this package's __init__). Exporters emitting
# `{'type': 'math', 'mode': ...}` must emit `blender_math` against this build --
# the C++ plugin's interface (a required `expr` string) is a different plugin
# and fails loudly on our property set.
mi.register_field('blender_math', lambda props: Math(props))
